...
## 1. Create an aspect test project with Caravela.TestFramework
...
1. The `CaravelaApplyAspects` project property is set to `False`, which disables the transformation of run-time code in the project. Aspects defined in the test project will still be compiled (i.e., the compile-time project will still be generated), but no aspect will be executed. This allows you to have a full design-time coding experience and, at the same time, to delay the execution of the aspect until the tests are run. You can enable compile-time application of aspects by setting the `CaravelaApplyAspects` to `True` in your `csproj` file, but you risk to get a more cumbersome experience because issues will appear at design and compilation time instead of when you run the test.
  
2. Expected test results (`*.t.cs`) are excluded from the compilation.

3. The Xunit test framework is customized.
...
[!code-csharp[Main](../../code/Caravela.Documentation.SampleCode.CompileTimeTesting/Aspects/SimpleLogAttribute.cs)]

To test this aspect, we create a test file with the following content:

[!code-csharp[Main](../../code/Caravela.Documentation.SampleCode.CompileTimeTesting/SimpleLogTests.cs)]
...
[!code-csharp[Main](../../code/Caravela.Documentation.SampleCode.CompileTimeTesting/SimpleLogTests.t.cs)]
...
## 4. Copy to the test output to the expected output
...
[!code-csharp[Main](../../code/Caravela.Documentation.SampleCode.CompileTimeTesting/_Runner.cs)]
